	...
	public:
		...
		public:
			...
			iterator_base(confighost& confighost, size_t config_index) :
				...
				m_index(recursive ? config_index == config::invalid_id || confighost.m_containers[config_index].size() == 0 ? config::invalid_id : 0 : 0),
				m_id(config_index) {}

			iterator_base<recursive>& operator++()
			{
				if constexpr (recursive) {
					if (m_confighost.m_containers[m_id].size() == ++m_index)
					{
						m_index = config::invalid_id;
						while (m_confighost.m_containers[m_id].id_parent_logical != config::invalid_id) {
							if (m_confighost.m_containers[m_id = m_confighost.m_containers[m_id].id_parent_inherited].size() != 0) {
								m_index = 0;
								break;
							}
						}
			...
			iterator_base<recursive> operator++(int) { iterator_base<recursive> retval = *this; ++(*this); return retval; }
			bool operator==(iterator_base<recursive> other) const { return m_index == other.m_index; }
			bool operator!=(iterator_base<recursive> other) const { return m_index != other.m_index; }
			value_type operator*()
			{
				return m_confighost.m_containers[m_confighost.m_containers[m_id][m_index]];
			}
		};
